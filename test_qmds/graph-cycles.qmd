...
Convert those steps into code. Drag the blocks into the loop body — initialisation is given. Press **Get Solution** if you get stuck, then **Play** or **Next step** to watch the search on the same graph. At each node the current node and all of its neighbours light up together; visited nodes, visited edges, and the bag update underneath.
...
Convert those steps into code. Drag the blocks into the **dfs** body — the colour setup is given. Press **Get Solution** if you get stuck, then **Play** or **Next step** to watch the 3-colour search on the same graph.
...
You are given a graph where each node is a country and each edge is a one-way flight. Check if there's a chance to get stuck in a cycle. If it is, return a set of edges as `key_edges` which should be reversed to break the cycle.

Fill in `around_the_world`, then press **Play** or **Step** to watch line-by-line. The flight map updates `visited_nodes`, `visited_edges`, `bag`, and `key_edges` as your code runs.

::: {.panel-grid}
::: {.panel}
<div id="around-world-editor"></div>
:::
::: {.panel}
<div id="around-world-viz" class="cb-viz-placeholder"></div>
:::
:::
...
import {
  aroundTheWorldStarterCode,
  aroundTheWorldSolutionCode,
  mountAroundTheWorldViz,
} from "./js/cycle-helpers.js";
```

```{ojs}
//| echo: false
aroundWorldEditor = PythonEditorInput.mountPlayable(
  document.querySelector("#around-world-editor"),
  {
    heading: "Python",
    defaultCode: aroundTheWorldStarterCode(),
    solutionCode: aroundTheWorldSolutionCode(),
  }
)
```
...
```{ojs}
//| echo: false
aroundWorldViz = mountAroundTheWorldViz(document.querySelector("#around-world-viz"), {
  width: 480,
  height: 280,
  editor: aroundWorldEditor,
})
```
