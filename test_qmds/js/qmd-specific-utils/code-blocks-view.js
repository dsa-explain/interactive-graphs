 ...
 *   preplaced?: string[],
 *   solutionOrder?: Array<string|{id: string, children?: Array}>,
...
export function mountCodeBlocksView(container, blocks = DEFAULT_BLOCKS, options = {}) {
  ...
  /**
   * Build a `.cb-block` element, either a draggable-but-unplaced palette
   * chip or a placed, deletable workspace block. The two only differ in:
   * a "palette" CSS modifier class, whether a delete button + nested drop
   * slot exist (palette chips are never placed, so neither applies), and
   * which of `draggedEl`/`draggedFromPalette` drag start/end wires up.
   */
  function buildBlockElement(def, { mode = "workspace" } = {}) {
    const isPalette = mode === "palette";
    const typeId = def.id;

    const el = document.createElement("div");
    el.className =
      "cb-block" + (isPalette ? " cb-palette-block" : "") + (def.container ? " cb-container" : "");
    el.draggable = true;
    el.dataset.type = typeId;
    if (!isPalette) el.dataset.id = `inst_${++instanceSeq}`;
    ...
    el.addEventListener("click", (e) => {
      if (isPalette) e.stopPropagation();
    ...
    el.addEventListener("dragstart", (e) => {
      if (isPalette) {
        draggedEl = null;
        draggedFromPalette = typeId;
      } else {
        draggedEl = el;
        draggedFromPalette = null;
      }
      e.dataTransfer.effectAllowed = "move";
      e.dataTransfer.setData("text/plain", isPalette ? typeId : el.dataset.id);
      requestAnimationFrame(() => el.classList.add("cb-dragging"));
    });

    el.addEventListener("dragend", () => {
      el.classList.remove("cb-dragging");
      if (isPalette) draggedFromPalette = null;
      else draggedEl = null;
      clearIndicators();
  ...
  function createBlockElement(typeId) {
    ...
    if (!def) return null;
    return buildBlockElement(def, { mode: "workspace" });
  }
  ...
  function renderPalette() {
    ...
    remaining.forEach((typeId) => {
      const def = defById.get(typeId);
      if (!def) return;
      const chip = buildBlockElement(def, { mode: "palette" });
      if (chip) palette.appendChild(chip);
    });
  }

  /**
   * Clear the workspace and re-place blocks for the given ordered list.
   * Each entry is either a plain type id (placed at this level) or
   * `{ id, children }` to recursively fill a container block's nested
   * slot — needed for solutions/arrangements that nest blocks inside
   * containers rather than leaving them flat in the workspace.
   */
  function applyArrangement(nodes) {
    workspace.innerHTML = "";
    used.clear();

    function placeInto(slotEl, list) {
      (list ?? []).forEach((node) => {
        const typeId = typeof node === "string" ? node : node?.id;
        if (!typeId || !defById.has(typeId) || used.has(typeId)) return;
        const block = createBlockElement(typeId);
        if (!block) return;
        slotEl.appendChild(block);
        used.add(typeId);

        const children = typeof node === "object" && node ? node.children : null;
        if (children) {
          const inner = block.querySelector(":scope > .cb-inner-slot");
          if (inner) placeInto(inner, children);
        }
      });
    }

    placeInto(workspace, nodes);
    renderPalette();
  }

  function applyDefaultArrangement() {
    applyArrangement(preplaced);
  }

  function applySolutionArrangement() {
    if (!solutionOrder) return;
    applyArrangement(solutionOrder);
  }
